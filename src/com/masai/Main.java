...
import com.masai.service.BusServicesImpl;
...
public class Main {
	...
	// admin functionality
	private static void adminFunctionality(Scanner sc, Map<Integer, Bus> products, Map<String, Passenger> customers,
			List<Transaction> transactions) throws InvalidDetailsException, ProductException, TransactionException {
		// admin login

		adminLogin(sc);

		BusService prodService = new BusServicesImpl();
	...
	public static String adminAddProduct(Scanner sc, Map<Integer, Bus> products, BusService prodService) {
		...
		Bus prod = new Bus(IDGeneration.generateId(), name, qty, price, cate);

		str = prodService.addBuses(prod, products);// considering all details are valid

		return str;

	}

	public static void adminViewAllProducts(Map<Integer, Bus> products, BusService prodService)
			throws ProductException {
		prodService.viewAllBuses(products);
	}

	public static void adminDeleteProduct(Scanner sc, Map<Integer, Bus> products, BusService prodService)
			throws ProductException {

		System.out.println("please enter the id of product to be deleted");
		int id = sc.nextInt();
		prodService.deleteBus(id, products);
	}

	public static String adminUpdateProduct(Scanner sc, Map<Integer, Bus> products, BusService prodService)
			throws ProductException {
		String result = null;
		...
		System.out.println("Enter the  Seat qty");
		int qty = sc.nextInt();

		System.out.println("Enter the Bus price/piece");
		double price = sc.nextDouble();

		System.out.println("Enter the Bus category");
		String cate = sc.next();

		Bus update = new Bus(id, name, qty, price, cate);

		result = prodService.updateBus(id, update, products);
		return result;
	}

	public static void adminViewAllCustomers(Map<String, Passenger> customers, CustomerService cusService)
			throws ProductException {
		List<Passenger> list = cusService.viewAllCustomers(customers);
	...
	public static void customerFunctionality(Scanner sc, Map<String, Passenger> customers,
	...
	public static void customerSignup(Scanner sc, Map<String, Passenger> customers) throws DuplicateDataException {
		...
		Passenger cus = new Passenger(balance, name, pass, address, email);
	...
	public static void customerLogin(String email,String pass, Map<String, Passenger> customers, CustomerService cusService)
	...
	public static void customerViewAllProducts(Map<Integer, Bus> products, BusService prodService)
			throws ProductException {
		prodService.viewAllBuses(products);
	}

	public static String customerBuyProduct(Scanner sc, String email, Map<Integer, Bus> products,
			Map<String, Passenger> customers, List<Transaction> transactions, CustomerService cusService)
	...
	public static String customerAddMoneyToWallet(Scanner sc, String email, Map<String, Passenger> customers,
	...
	public static double customerViewWalletBalance(String email, Map<String, Passenger> customers,
			CustomerService cusService) {
		double walletBalance = cusService.viewWalletBalance(email, customers);
		return walletBalance;
	}

	public static void customerViewMyDetails(String email, Map<String, Passenger> customers,
			CustomerService cusService) {
		Passenger cus = cusService.viewCustomerDetails(email, customers);
...
//file check
		Map<Integer, Bus> products = FileExists.productFile();
